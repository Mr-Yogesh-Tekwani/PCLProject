...
class SelectVehicleViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource{

    weak var delegate : VehicleDelegate?
    
    var dataForDropdown: GetAvailableVehicleArray?
    ...
    func pickerView(_ pickerView: UIPickerView, titleForRow row: Int, forComponent component: Int) -> String? {
        return dataForDropdown?[row].plateNumber ?? ""
    }

    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        selectedOption = dataForDropdown?[row].plateNumber ?? ""
        dropdownTextField.text = dataForDropdown?[row].plateNumber ?? ""
    }
    ...
    @objc private func doneButtonTapped() {
        ...
        self.dismiss(animated: true, completion:{
            self.delegate?.vehicleDataRecieved(self, didReturnData: selectedOption)
        })
    }
