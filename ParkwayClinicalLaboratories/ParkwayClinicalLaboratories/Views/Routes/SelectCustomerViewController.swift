...
class SelectCustomerViewController: UIViewController  , UIPickerViewDelegate, UIPickerViewDataSource {
    
    weak var delegate : CustomerDelegate?
    
    var dataForDropdown: GetCustomerArray?
    ...
    func pickerView(_ pickerView: UIPickerView, titleForRow row: Int, forComponent component: Int) -> String? {
        return dataForDropdown?[row].customerName ?? ""
    }

    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        
        selectedOption = dataForDropdown?[row].customerName ?? ""
        cuid = dataForDropdown?[row].customerID
        dropdownTextField.text = dataForDropdown?[row].customerName ?? ""
    }
    ...
    @objc private func doneButtonTapped() {
        ...
        guard let selectedOption = selectedOption, let cuid = cuid else {
            return
        }
        
        dropdownTextField.resignFirstResponder()
        self.dismiss(animated: true, completion: {
            self.delegate?.customerDataRecieved(self, didReturnData: selectedOption, cid: cuid)
        })
    }
